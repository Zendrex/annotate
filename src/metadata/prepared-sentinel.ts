...
/** True if `prepare(ctor)` previously completed with no pending work and no enqueue has invalidated that state. */
export function isFullyPrepared(ctor: Ctor): boolean {
	return fullyPrepared.has(ctor);
}

/** Call only after `prepare` drained all pending work for the ctor and its ancestor chain. */
export function markFullyPrepared(ctor: Ctor): void {
	fullyPrepared.add(ctor);
}

/** Drops the fully-prepared mark for `ctor`. No-op if not currently marked. */
