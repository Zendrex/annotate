...
describe("createClassDecorator (Stage-3)", () => {
	...
	// Skipped until L2 rewrites the reflector — `Tag.reflect(X).class()` depends on Phase L.
	test.skip("appends per application; first wins on metadata()", () => {
		const Tag = createClassDecorator<string>();
	...
	test("unique:true throws on second application to same class", () => {
		...
		expect(() => {
			...
			@Tag("b")
			class X {}
			// biome-ignore lint/complexity/noVoid: discard class reference to silence unused-variable without export
			void X;
